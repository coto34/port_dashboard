...
def calculate_roi(
    ...
    try:
        ...
        if inv <= 0 or current_teu <= 0 or cur_p <= 0 or tgt_p <= 0:
            return {'annual_savings':0.0,'total_savings_3_years':0.0,'roi_percentage':float('nan'),'payback_years':float('inf'),'productivity_improvement_pct':float('nan'),'cost_reduction_per_teu':0.0}
...
selected_metric = st.sidebar.selectbox("Seleccionar Métrica Principal:", options=list(metric_options.keys()), format_func=lambda x: metric_options[x], index=0)

selected_chilean_ports = st.sidebar.multiselect("Puertos Chilenos a Comparar:", options=chilean_ports['Puerto'].tolist(), default=['Puerto Coronel','Puerto Valparaíso','Puerto San Antonio'])
selected_top_ports = st.sidebar.multiselect("Top Performers a Comparar:", options=top_performers['Puerto'].tolist(), default=['Yangshan (China)','Singapur','Algeciras (España)'])
...
with col1:
    ...
    if not filtered_data.empty:
        fig_bar = px.bar(
            filtered_data, x='Puerto', y=selected_metric, color='Port_Type',
        ...
        fig_bar.update_layout(xaxis_tickangle=-45, height=450, showlegend=True, xaxis_title="Puertos", yaxis_title=metric_options[selected_metric])
...
with col2:
    ...
    if selected_chilean_ports and not filtered_chilean.empty:
        ...
        st.metric("CPPI Score Promedio", f"{avg_cppi:.1f}", delta=f"{avg_cppi - top_cppi_avg:.1f}", help="Container Port Performance Index - World Bank (indicar año del dato)")
        st.metric("Productividad Promedio", f"{avg_productivity:.1f} moves/hr", delta=f"{avg_productivity - top_productivity_avg:.1f}", help="Movimientos de contenedores por hora")
        st.metric("Costo Promedio por TEU", f"${avg_cost:.0f}", delta=f"${avg_cost - top_cost_avg:.0f}", delta_color="inverse", help="Costo operacional por contenedor")
...
with col_roi1:
    ...
    target_port = st.selectbox("Puerto a Analizar:", options=selected_chilean_ports if selected_chilean_ports else ['Puerto Valparaíso'], index=0)
    if target_port:
...
with col_roi2:
    ...
    investment_amount = st.number_input("Inversión Total (USD):", min_value=100000, max_value=100000000, value=5000000, step=500000, help="Inversión total del proyecto de modernización")
    if target_port:
        current_productivity = float(port_data['Berth_Productivity'])
        target_productivity = st.slider("Productividad Objetivo (moves/hr):", min_value=max(1, int(current_productivity)), max_value=50, value=min(int(current_productivity * 1.5), 45), help=f"Productividad actual: {current_productivity:.1f} moves/hr")
        reference_port = st.selectbox("Modelo de Referencia:", options=['Yangshan (China)','Singapur','Algeciras (España)','Rotterdam (Países Bajos)'], index=0)
        st.markdown("#### Supuestos (conservadores)")
        coverage_pct = st.slider("Cobertura del proyecto (% del TEU impactado)", 0, 100, 60, help="Porción del volumen realmente afectado")
        adoption_pct = st.slider("Adopción/uso efectivo (%)", 0, 100, 70)
        savings_factor_pct = st.slider("Factor de eficiencia (%)", 0, 100, 50, help="Coeficiente conservador que traduce productividad en ahorro")
...
"""
    ...
    if target_port:
        ...
        st.caption(f"ROI a {years_horizon} años (texto): {safe_pct(roi_results['roi_percentage'])}")
...
with col_gap1:
    ...
    if selected_chilean_ports and selected_top_ports and not filtered_chilean.empty and not filtered_top.empty:
        chilean_avg = filtered_chilean[['CPPI_Score','Berth_Productivity','Automation_Level','Rail_Connectivity','Digital_Systems']].mean()
        top_avg = filtered_top[['CPPI_Score','Berth_Productivity','Automation_Level','Rail_Connectivity','Digital_Systems']].mean()
        metrics_radar = ['CPPI Score','Productividad','Automatización','Ferrocarril','Digital']
        fig_radar = go.Figure()
        fig_radar.add_trace(go.Scatterpolar(r=[chilean_avg['CPPI_Score']/150*100, chilean_avg['Berth_Productivity']/50*100, chilean_avg['Automation_Level'], chilean_avg['Rail_Connectivity'], chilean_avg['Digital_Systems']], theta=metrics_radar, fill='toself', name='Puertos Chilenos', line=dict(color='#ff7f0e', width=2)))
        fig_radar.add_trace(go.Scatterpolar(r=[top_avg['CPPI_Score']/150*100, top_avg['Berth_Productivity']/50*100, top_avg['Automation_Level'], top_avg['Rail_Connectivity'], top_avg['Digital_Systems']], theta=metrics_radar, fill='toself', name='Top Performers', line=dict(color='#1f77b4', width=2)))
...
with col_gap2:
    ...
    if selected_chilean_ports and not filtered_chilean.empty:
        ...
        for metric in ['Automation_Level','Rail_Connectivity','Digital_Systems','Berth_Productivity']:
            ...
            gaps_analysis.append({'metric': metric_name,'gap_percentage': gap_percentage,'gap_absolute': gap_absolute,'priority': priority,'css_class': css_class})
...
if year_filter != 2023:
    ...
    with col_trend1:
        ...
        if 'Puerto Valparaíso' in historical_data:
            ...
            fig_trend = go.Figure()
            fig_trend.add_trace(go.Scatter(x=years, y=cppi_values, mode='lines+markers', name='CPPI Score', line=dict(color='#ff7f0e', width=3), marker=dict(size=8)))
...
"""
        ...
        st.download_button(label="📥 Descargar Reporte TXT", data=executive_summary, file_name=f"reporte_ejecutivo_puertos_{datetime.now().strftime('%Y%m%d')}.txt", mime="text/plain")
...
with col_export2:
    ...
    if st.button("📈 Exportar Excel", use_container_width=True):
        ...
        excel_buffer.seek(0)
        st.download_button(label="📥 Descargar Excel", data=excel_buffer.getvalue(), file_name=f"analisis_puertos_completo_{datetime.now().strftime('%Y%m%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
"""
        st.download_button(label="📥 Descargar ROI", data=roi_text, file_name=f"analisis_roi_{target_port.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.txt", mime="text/plain")

with col_export4:
    st.subheader("📋 Datos CSV")
    if st.button("📊 Exportar CSV", use_container_width=True):
        csv_data = filtered_data.to_csv(index=False)
        st.download_button(label="📥 Descargar CSV", data=csv_data, file_name=f"datos_puertos_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv")
