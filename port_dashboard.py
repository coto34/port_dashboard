...
<style>
    ...
    .priority-card {
        background:#fff; padding:1rem; border-radius:8px; border-left:4px solid #007bff;
...
def classify_gap(gap_percentage):
    if gap_percentage >= 60: return "CRÍTICO", "gap-critical"
    elif gap_percentage >= 40: return "ALTO", "gap-high"
    elif gap_percentage >= 20: return "MEDIO", "gap-medium"
    else: return "BAJO", "gap-low"
...
all_ports, top_performers, chilean_ports, historical_data = load_data()
...
metric_options = {
    'CPPI_Score':'CPPI Score','TEU_Annual':'TEU Anual','Berth_Productivity':'Productividad de Muelle (moves/hr)',
    'Time_in_Port_Hours':'Tiempo en Puerto (hrs)','Automation_Level':'Nivel de Automatización (%)',
    'Rail_Connectivity':'Conectividad Ferroviaria (%)','Digital_Systems':'Sistemas Digitales (%)',
    'Cost_Per_TEU':'Costo por TEU (USD)','Dwell_Time_Days':'Tiempo de Estadía (días)'
}
...
if st.sidebar.button("♻️ Limpiar caché de datos"):
    st.cache_data.clear()
...
# =========================
missing_top = sorted(set(selected_top_ports) - set(top_performers['Puerto'].tolist()))
if missing_top:
    st.warning(f"Algunos Top Performers seleccionados no existen en el dataset: {', '.join(missing_top)}")

# Subconjuntos filtrados (copias seguras)
filtered_chilean = chilean_ports[chilean_ports['Puerto'].isin(selected_chilean_ports)].copy()
filtered_top = top_performers[top_performers['Puerto'].isin(selected_top_ports)].copy()
...
with col1:
    st.subheader(f"📊 Comparación: {metric_options[selected_metric]} (CPPI {year_filter} donde corresponda)")

    if filtered_chilean.empty and filtered_top.empty:
        st.info("Selecciona al menos un puerto chileno o un top performer para graficar.")
    else:
        # Concatenar y etiquetar
        parts = []
        if not filtered_top.empty:
            a = filtered_top.copy()
            a['Port_Type'] = 'Top Performer'
            parts.append(a)
        if not filtered_chilean.empty:
            b = filtered_chilean.copy()
            b['Port_Type'] = 'Chilean Port'
            parts.append(b)
        filtered_data = pd.concat(parts, ignore_index=True)

        # Asegurar orden por tu selección: primero Top, luego Chile
        ordered_ports = selected_top_ports + selected_chilean_ports
        ...
        fig_bar.update_layout(
            ...
            xaxis=dict(categoryorder='array', categoryarray=ordered_ports)
        )
...
with col2:
    ...
    # Toggle: alcance de KPIs (promedio vs puerto individual)
    kpi_mode = st.radio("¿Qué mostrar?", ["Promedio seleccionados (Chile)", "Puerto individual"], horizontal=False)

    # Toggle: referencia del Δ (benchmarks seleccionados vs promedio top global)
    delta_scope = st.radio("Comparar contra:", ["Benchmarks seleccionados", "Promedio top global"], horizontal=False)

    # Promedios de referencia (top)
    if delta_scope == "Benchmarks seleccionados" and not filtered_top.empty:
        ref_top = filtered_top
        ref_note = "Δ vs *benchmarks seleccionados*"
    else:
        ref_top = top_performers
        ref_note = "Δ vs *promedio global de top performers*"

    top_cppi_avg = ref_top['CPPI_Score'].mean()
    top_productivity_avg = ref_top['Berth_Productivity'].mean()
    top_cost_avg = ref_top['Cost_Per_TEU'].mean()

    if kpi_mode == "Promedio seleccionados (Chile)":
        if filtered_chilean.empty:
            st.info("Selecciona puertos chilenos en el panel izquierdo.")
        else:
            ...
            st.metric("CPPI Score (prom.)", f"{avg_cppi:.1f}",
                      delta=f"{avg_cppi - top_cppi_avg:.1f}",
                      help="Container Port Performance Index - World Bank (indicar año del dato)")
            st.metric("Productividad (prom.)", f"{avg_productivity:.1f} moves/hr",
                      delta=f"{avg_productivity - top_productivity_avg:.1f}",
                      help="Movimientos de contenedores por hora")
            st.metric("Costo por TEU (prom.)", f"{avg_cost:.0f}",
                      delta=f"{avg_cost - top_cost_avg:.0f}",
                      delta_color="inverse",
                      help="Costo operacional por contenedor")

            st.caption(f"KPIs = promedio de **puertos chilenos seleccionados**; {ref_note}.")
    else:
        # Puerto individual para KPIs (independiente del ROI)
        if filtered_chilean.empty:
            st.info("Selecciona puertos chilenos para elegir uno individual.")
        else:
            kpi_port = st.selectbox("Puerto chileno (KPIs):", options=filtered_chilean['Puerto'].tolist(), index=0)
            prow = filtered_chilean[filtered_chilean['Puerto'] == kpi_port].iloc[0]

            st.metric("CPPI Score", f"{prow['CPPI_Score']:.1f}",
                      delta=f"{prow['CPPI_Score'] - top_cppi_avg:.1f}")
            st.metric("Productividad", f"{prow['Berth_Productivity']:.1f} moves/hr",
                      delta=f"{prow['Berth_Productivity'] - top_productivity_avg:.1f}")
            st.metric("Costo por TEU", f"{prow['Cost_Per_TEU']:.0f}",
                      delta=f"{prow['Cost_Per_TEU'] - top_cost_avg:.0f}",
                      delta_color="inverse")
            st.caption(f"KPIs del puerto **{kpi_port}**; {ref_note}.")
...
with col_roi1:
    ...
    roi_port_options = filtered_chilean['Puerto'].tolist() if not filtered_chilean.empty else chilean_ports['Puerto'].tolist()
    target_port = st.selectbox("Puerto a Analizar:", options=roi_port_options, index=0)
...
with col_roi2:
    st.subheader("🎯 Objetivos y Supuestos")
    ...
    coverage_pct = st.slider("Cobertura del proyecto (% TEU impactado)", 0, 100, 60)
    adoption_pct = st.slider("Adopción/uso efectivo (%)", 0, 100, 70)
    savings_factor_pct = st.slider("Factor de eficiencia (%)", 0, 100, 50, help="Traslada productividad a ahorro")
...
with col_roi3:
    ...
    def calculate_roi(
        ...
        try:
            ...
            if inv <= 0 or current_teu <= 0 or cur_p <= 0 or tgt_p <= 0:
                return {'annual_savings':0.0,'total_savings_n_years':0.0,'roi_percentage':float('nan'),'payback_years':float('inf'),'productivity_improvement_pct':float('nan'),'cost_reduction_per_teu':0.0}
    ...
    # Tarjeta ROI aislada (evita override de tema)
...
<style>
  ...
  body {{ margin:0; font-family: system-ui, -apple-system, Segoe UI, Roboto, Ubuntu, Cantarell, 'Helvetica Neue', Arial, 'Noto Sans', 'Liberation Sans', sans-serif; color:#111; background:transparent; }}
  .card {{ background:{bg}; border-left:8px solid {border}; padding:16px; border-radius:10px; margin:6px 0; box-shadow:0 2px 6px rgba(0,0,0,0.08); }}
  .label {{ font-size:0.95rem; opacity:0.85; margin-bottom:6px; color:#111; }}
  .value {{ font-size:2rem; font-weight:800; line-height:1; color:{txt_color}; -webkit-text-fill-color:{txt_color}; }}
</style>
</head>
<body>
  <div class="card">
    <div class="label">💹 ROI a {horizon} años</div>
...
"""
    ...
    impr = safe_pct(roi_results['productivity_improvement_pct'])
    st.metric("Mejora Productividad", f"+{impr}" if impr != "N/D" else impr)
...
with col_gap1:
    ...
    if not filtered_chilean.empty and (not filtered_top.empty):
        ...
        fig_radar.add_trace(go.Scatterpolar(
            r=[ch_avg['CPPI_Score']/150*100, ch_avg['Berth_Productivity']/50*100, ch_avg['Automation_Level'], ch_avg['Rail_Connectivity'], ch_avg['Digital_Systems']],
            theta=metrics_radar, fill='toself', name='Puertos Chilenos', line=dict(color='#ff7f0e', width=2)
        ))
        fig_radar.add_trace(go.Scatterpolar(
            r=[tp_avg['CPPI_Score']/150*100, tp_avg['Berth_Productivity']/50*100, tp_avg['Automation_Level'], tp_avg['Rail_Connectivity'], tp_avg['Digital_Systems']],
            theta=metrics_radar, fill='toself', name='Top Performers', line=dict(color='#1f77b4', width=2)
        ))
        fig_radar.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 100])), showlegend=True, title="Análisis Multidimensional - Normalizado 0-100%", height=400)
        st.plotly_chart(fig_radar, use_container_width=True)
    else:
        st.info("Para el radar, selecciona al menos un puerto chileno y un top performer.")

with col_gap2:
    st.subheader("🚨 Prioridades de Mejora")
    if not filtered_chilean.empty:
        gaps_analysis = []
        for metric in ['Automation_Level','Rail_Connectivity','Digital_Systems','Berth_Productivity']:
            ch_avg = filtered_chilean[metric].mean()
            tp_avg = filtered_top[metric].mean() if not filtered_top.empty else top_performers[metric].mean()
            gap_abs = tp_avg - ch_avg
            gap_pct = (gap_abs / tp_avg * 100) if tp_avg > 0 else 0
            priority, css_class = classify_gap(gap_pct)
            metric_name = {'Automation_Level':'Automatización','Rail_Connectivity':'Conectividad Ferroviaria','Digital_Systems':'Sistemas Digitales','Berth_Productivity':'Productividad'}[metric]
            gaps_analysis.append({'metric': metric_name,'gap_percentage': gap_pct,'gap_absolute': gap_abs,'priority': priority,'css_class': css_class})
        gaps_analysis.sort(key=lambda x: x['gap_percentage'], reverse=True)

        for i, g in enumerate(gaps_analysis):
            priority_color = {'CRÍTICO':'#dc3545','ALTO':'#fd7e14','MEDIO':'#ffc107','BAJO':'#28a745'}.get(g['priority'], '#6c757d')
            st.markdown(f"""
            <div class="priority-card" style="border-left-color:{priority_color};">
                <h5 style="margin-bottom:0.5rem;color:#333;">{i+1}. {g['metric']}</h5>
                <p style="margin-bottom:0.5rem;">
                    Gap: <strong>{g['gap_percentage']:.1f}%</strong> 
                    <span class="{g['css_class']}">{g['priority']}</span>
                </p>
                <div style="background:#e9ecef;border-radius:4px;height:8px;overflow:hidden;">
                    <div style="background:{priority_color};height:100%;width:{min(g['gap_percentage'],100):.1f}%;"></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
    else:
        st.info("Selecciona puertos chilenos para ver prioridades de mejora.")
...
if year_filter != 2023:
    ...
    with col_trend1:
        ...
        if 'Puerto Valparaíso' in historical_data:
            ...
            fig_trend = go.Figure()
            fig_trend.add_trace(go.Scatter(x=years, y=cppi_values, mode='lines+markers', name='CPPI Score',
                                           line=dict(color='#ff7f0e', width=3), marker=dict(size=8)))
            fig_trend.update_layout(title="Evolución CPPI Score 2020-2023", xaxis_title="Año", yaxis_title="CPPI Score", height=300)
    ...
    with col_trend2:
        ...
        if 'Puerto Valparaíso' in historical_data:
            ...
            for i in range(1, len(years)):
                prev_value = cppi_values[i-1]; curr_value = cppi_values[i]
            ...
            projected_2024 = cppi_values[-1] * (avg_growth/100 + 1 if math.isfinite(avg_growth) else 1)
            st.write(f"**Proyección 2024:** CPPI Estimado: {projected_2024:.1f}")
...
if 'filtered_data' in locals() and not filtered_data.empty:
    ...
    styled_data = display_data.style.format({
        'CPPI_Score':'{:.1f}','TEU_Annual':'{:,}','Berth_Productivity':'{:.1f}',
        'Time_in_Port_Hours':'{:.1f}','Cost_Per_TEU':'${:.0f}','Dwell_Time_Days':'{:.1f}'
    })
    st.dataframe(styled_data, use_container_width=True)
else:
    st.info("No hay datos para mostrar en la tabla. Ajusta la selección en el panel izquierdo.")
...
PUERTOS ANALIZADOS:
• Chilenos: {', '.join(selected_chilean_ports) if selected_chilean_ports else '(ninguno)'}
• Benchmarks: {', '.join(selected_top_ports) if selected_top_ports else '(ninguno)'}
"""
        if 'gaps_analysis' in locals() and gaps_analysis:
            executive_summary += "\nRESUMEN DE BRECHAS CRÍTICAS:\n"
            for i, gap in enumerate(gaps_analysis[:3]):
                executive_summary += f"{i+1}. {gap['metric']}: gap {gap['gap_percentage']:.1f}%, prioridad {gap['priority']}\n"

        if 'roi_results' in locals():
            executive_summary += f"""
ANÁLISIS ROI ({target_port}):
• Inversión: {safe_money(investment_amount)}
• ROI {years_horizon} años: {safe_pct(roi_results['roi_percentage'])}
...
"""
        st.text_area("📄 Reporte Ejecutivo Generado:", executive_summary, height=380)
        st.download_button(label="📥 Descargar Reporte TXT", data=executive_summary, file_name=f"reporte_ejecutivo_{datetime.now().strftime('%Y%m%d')}.txt", mime="text/plain")
            ...
            with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                ...
                if historical_data:
                    hist_df = pd.DataFrame([
                        {'Puerto': p, 'Año': a, **d}
                        for p, yd in historical_data.items()
                        for a, d in yd.items()
                    ])
                    hist_df.to_excel(writer, sheet_name='Datos_Historicos', index=False)
            excel_buffer.seek(0)
            st.download_button(label="📥 Descargar Excel", data=excel_buffer.getvalue(),
                               file_name=f"analisis_puertos_{datetime.now().strftime('%Y%m%d')}.xlsx",
                               mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        except Exception as e:
            st.error(f"No se pudo generar Excel (¿falta openpyxl?). Error: {e}")
...
"""
        st.download_button(label="📥 Descargar ROI", data=roi_text,
                           file_name=f"analisis_roi_{target_port.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.txt",
                           mime="text/plain")

with col_export4:
    st.subheader("📋 Datos CSV")
    if 'filtered_data' in locals() and not filtered_data.empty and st.button("📊 Exportar CSV", use_container_width=True):
        csv_data = filtered_data.to_csv(index=False)
        st.download_button(label="📥 Descargar CSV", data=csv_data,
...
- **Payback**: cálculo conservador (flujo a N años; caso-dependiente)
""")
...
if st.sidebar.checkbox("🔧 Modo Debug", value=False):
    ...
    st.sidebar.write(f"Puertos chilenos seleccionados: {len(selected_chilean_ports)} -> {selected_chilean_ports}")
    st.sidebar.write(f"Top performers seleccionados: {len(selected_top_ports)} -> {selected_top_ports}")
    st.sidebar.write(f"Chilean filtrados: {len(filtered_chilean)} | Top filtrados: {len(filtered_top)}")
